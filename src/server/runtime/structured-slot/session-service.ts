/**
 * Structure session service — the structure-session state and dispatch-guard
 * foundation (Task 12, design §11.3, spec §9.1).
 *
 * - `StructureSessionState` is the current turn's structure session: the
 *   session kind, the bound grant, the proposal/candidate reference and the
 *   `completion`. A formed candidate means the session is LOCKED (no more
 *   write/submit) and the completion is `structure_commit_candidate_created`;
 ...
 *   validated against the session completion. After the structure candidate
 *   only `send_message` (the completion dispatch) is legal; `request_human_input`
 *   remains the exclusive abandon exit and is always legal before the
 *   completion dispatch. Task 14 wires this into the tool/action pipeline.
 * - `getStructureStatus(grant)` is the lifecycle summary (open /
 *   candidate_created / committed / abandoned) reconciling TaskEvents over the
 *   private journal — Task 7 authority, never the private store alone.
...
import type { ProposalLifecycle, ProposalView, StructureCommitCandidate } from '../../storage/structured-slot-private-store';
...
/**
 * Dispatch-guard foundation for structure (design §11.3): before a ForgeAction
 * is proposed to the ActionBuffer, validate it against the session completion.
 *
 * - After `structure_commit_candidate_created` only `send_message` (the
 *   completion dispatch) is legal; `request_human_input` remains the exclusive
 *   abandon exit.
 ...
 */
export function assertStructuredForgeAction(state: StructureSessionState, action: ForgeAction): StructureGuardResult {
  if (action.type === 'request_human_input') {
    return { ok: true };
  }
  if (state.completion === 'structure_commit_candidate_created' && action.type === 'send_message') {
    return { ok: true };
  }
  return {
    ok: false,
    ...
    reason:
      state.completion === null
        ? 'a structure session may only end in send_message after a candidate is formed, or abandon via request_human_input'
        : 'after structure_commit_candidate_created only send_message is legal (request_human_input remains the abandon exit)',
  };
}

...
describe('TaskScheduler progress guard structured decision (spec §11)', () => {
  ...
  it('repairs a crash between supersede and synthesize for a continue decision (spec §11.6)', async () => {
    const { harness, reviewerInputId } = await parkedHarness();
    // Simulate a crash AFTER human_answered + pending_inputs_superseded but
    // BEFORE the synthesized continue input.
    const committed = await harness.environment.events.read(harness.taskId);
    const pendingRequest = committed
      .map((entry) => entry.event)
      .filter((e): e is Extract<TaskEvent, { type: 'human_requested' }> => e.type === 'human_requested')
      .at(-1)!;
    await harness.environment.events.append(
      harness.taskId,
      makeTaskEvent({
        ...
        node: makeEventNode({
          sequence: 100,
          agentId: pendingRequest.node.agentId,
          kind: 'human_answer',
          title: pendingRequest.node.title,
          body: '请直接提交当前版本',
        }),
        answer: '请直接提交当前版本',
        decision: 'continue',
    ...
    await harness.environment.events.append(
      harness.taskId,
      makeTaskEvent({ type: 'pending_inputs_superseded', supersededNodeIds: [reviewerInputId] }),
    );
    // No synthesize input: the crash point.

    // A fresh scheduler over the same roots simulates the restart; resume must
    // heal the half-state deterministically (no duplicate supersede, exactly
    // one synthesized input).
    const restarted = new TaskScheduler({
      service: harness.environment.service,
      runner: harness.environment.service.runner,
      runtime: harness.recording,
    ...
    await restarted.recoverInterruptedTasks();
    await restarted.resume(harness.taskId);
    ...
    expect(synth.node.body).toBe('请直接提交当前版本');
    expect(synth.node.humanAuthorized).toBeUndefined();
    const synths = (await harness.environment.events.read(harness.taskId)).filter(
      (entry) => entry.event.type === 'agent_input' && entry.event.id.startsWith('synthesize-'),
    );
    expect(synths).toHaveLength(1);
    const finalWorkspace = await harness.environment.service.getWorkspace(harness.taskId);
    expect(finalWorkspace.task.status).toBe('completed');
  });

  it('repairs a crash between supersede and synthesize for an accept decision (spec §11.6)', async () => {
    const { harness, reviewerInputId } = await parkedHarness();
    const committed = await harness.environment.events.read(harness.taskId);
    const pendingRequest = committed
      .map((entry) => entry.event)
    ...
    // The accept repair synthesizes for the final submitter with the latest
    // version and humanAuthorized=true — generated by the platform, never by
    // the model (spec §7.1 closedness).
    const synth = await findSynthesized(harness, 'accept');
    expect(synth.node.agentId).toBe('reviewer');
    expect(synth.node.inputVersion).toBe(2);
    expect(synth.node.humanAuthorized).toBe(true);
